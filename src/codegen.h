...
static void emit_imports(AstNode *node) {
    if (!node) return;
    if (node->kind == AST_IMPORT) {
        fprintf(out, "#include %s\n", node->import.path);
    } else if (node->kind == AST_BLOCK) {
        for (int i = 0; i < node->block.count; i++) {
            emit_imports(node->block.stmts[i]);
        }
    }
}

static void emit_macros(AstNode *node) {
    if (!node) return;
    if (node->kind == AST_MACRO_DEF) {
        fprintf(out, "#define %s %s\n", node->macro_def.name, node->macro_def.value);
    } else if (node->kind == AST_BLOCK) {
        for (int i = 0; i < node->block.count; i++) {
            emit_macros(node->block.stmts[i]);
        }
    }
}
...
void codegen_generate(AstNode *program, const char *input_file, FILE *output) {
    ...
    for (int i = 0; i < program->program.count; i++) {
        emit_imports(program->program.nodes[i]);
    }
    fprintf(out, "\n");

    for (int i = 0; i < program->program.count; i++) {
        emit_macros(program->program.nodes[i]);
    }
    ...
    for (int i = 0; i < program->program.count; i++) {
        ...
        if (node->kind == AST_BLOCK) {
            ...
            for (int j = 0; j < node->block.count; j++) {
                AstNodeKind k = node->block.stmts[j]->kind;
                if (k != AST_IMPORT) {
                    all_imports = false;
                }
                if (k == AST_FUNC_DEF || k == AST_STRUCT_DEF ||
                    k == AST_ENUM_DEF || k == AST_UNION_DEF ||
                    k == AST_MACRO_DEF) {
                    has_top_level = true;
                }
            }
            ...
            if (has_top_level) {
                for (int j = 0; j < node->block.count; j++) {
                    ...
                    switch (child->kind) {
                        ...
                        case AST_MACRO_DEF: break;
                        case AST_FUNC_DEF: gen_func_def(child, NULL); break;
                        case AST_STRUCT_DEF:
                        case AST_ENUM_DEF:
                        case AST_UNION_DEF:
                            gen_type_decl(child);
    ...
    for (int i = 0; i < program->program.count; i++) {
        ...
        if (node->kind == AST_BLOCK) {
            for (int j = 0; j < node->block.count; j++) {
                AstNode *child = node->block.stmts[j];
                if (child->kind == AST_FUNC_DEF &&
                    strcmp(child->func_def.name, "main") == 0) {
                    has_main = true;
                    break;
                }
            }
